...
VIDEO_PATH = 'video/test/test_traffic.mp4'
...
def draw_mask(height, width):
    ...
    # 初始化2个撞线polygon  蓝色
    # 蓝色多边形坐标，可根据自己的场景修改
    list_pts_blue = [[point_1[0], point_1[1]-40], [point_2[0], point_2[1]-40], [point_2[0], point_2[1]], [point_1[0], point_1[1]]]
    ...
    mask_image_temp = np.zeros((height, width), dtype=np.uint8)
    # 黄色多边形坐标，可根据自己的场景修改
    list_pts_yellow = [[point_1[0], point_1[1]+40], [point_2[0], point_2[1]+40], [point_2[0], point_2[1]], [point_1[0], point_1[1]]]
...
def count_car(width=1364, height=768):
    # 根据视频尺寸，填充供撞线计算使用的polygon
    # 1364,768

    polygon_mask_blue_and_yellow, color_polygons_image = draw_mask(height, width)
    # list与蓝色polygon重叠
    ...
    while True:
        ...
        # 更新跟踪器
        output_image_frame, _, list_bboxs, id_tracker = tracker.update_tracker(detector, im, id_tracker)
        # 输出图片
